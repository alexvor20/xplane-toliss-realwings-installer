...
def _build_all_wings_objects(include_frames: bool = True) -> tuple[
    list[ACFObject], list[ACFObject], list[ACFObject]
]:
    """Return (CEO_wingtips, CEO_sharklets, NEO) object lists."""

    frames_lines = [] if not include_frames else [
        _obj("RealWings321/Frames321.obj", FLAGS_LIGHTNING_INSIDE, x=6.00, y=2.06, z=60.98),
        _obj("RealWings321/Lines321.obj",  FLAGS_LIGHTNING_INSIDE, x=6.00, y=2.06, z=60.98),
    ]

    ceo_body = [
        _obj("RealWings321/Main.obj",      FLAGS_ALL_VIEWS),
        _obj("RealWings321/Glass.obj",     FLAGS_LIGHTNING_GLASS_OUTSIDE),
        _obj("RealWings321/Secondary.obj", FLAGS_ALL_VIEWS),
    ]
    neo_body = [
        _obj("RealWings321/MainNEO.obj",      FLAGS_ALL_VIEWS),
        _obj("RealWings321/GlassNEO.obj",     FLAGS_LIGHTNING_GLASS_OUTSIDE),
        _obj("RealWings321/SecondaryNEO.obj", FLAGS_ALL_VIEWS),
    ]
...
def main() -> None:
    ...
    print(" RealWings 321 Installer v1.0")
    print("=" * SEPARATOR_WIDTH)
    ...
    if not ask_yes_no("Do you want to install RealWings321?"):
        input("\nPress Enter to exit...")
        sys.exit(1)
    ...
    ) = _build_all_wings_objects(include_frames=install_frames)

    try:
        # ── Backup: original a321.acf  ───────────────────────────────────────
